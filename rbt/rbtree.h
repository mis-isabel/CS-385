/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Sean Payba and Isabel Sutedjo
 * Version     : 1.0
 * Date        : 11/26/2022
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while(x != nullptr){
            y = x;
            if(find(key) != end()){ //check for duplicates and delete the duplicate nodes
                std::stringstream dupe;
                dupe << "Attempt to insert duplicate key '" << z->key() << "'.";
                delete z;
                throw tree_exception(dupe.str());
            }
            else if(z->key() < x->key()){
                x = x->left;
            }
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        // TODO
        if(n == nullptr){ // delete node if it is null
            delete n;
        }
        else{
            if(n->left != nullptr){ // check left side and delete
                delete_tree(n->left);
            }

            if(n->right != nullptr){ // check right side and delete
                delete_tree(n->right);
            }
    ...
    int height(Node<K, V> *node) const {
        ...
        if(node == nullptr){
            ...
            lh = lh + height(node->left);
            rh = rh + height(node->right);
            // use larger value for height
            if(lh >= rh){
                return lh;
            }
            else{
    ...
    size_t leaf_count(Node<K, V> *node) const {
        ...
        // check if it's a leaf by seeing if right and left are empty
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        // TODO
        // calculate the total nodes - leaves of the tree = non leafs
        size_t count = size_ - leaf_count(node);
        return count;
    }
    ...
    int diameter(Node<K, V> *node) const {
        // TODO
        int lh, rh, ld, rd = 0;
        if(node == nullptr){ // check for empty nodes
            return 0;
        }
        else{
            //traverse left and right sides of tree
            lh = height(node->left)+1;
            rh = height(node->right)+1;

            //diameters of left and right subtree
            ld = diameter(node->left);
            rd = diameter(node->right);

            int maxd = std::max(ld, rd);
            int sumh = lh+rh;
            return std::max(maxd, sumh);
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        if(level == 0){
            ...
            // finds width of a certain level
    ...
    size_t null_count(Node<K, V> *node) const {
        // TODO
        size_t count = 1;
        // check if the tree is empty, then return 1
        if (node == nullptr){
            return count;
        }
        // else add null_count to both left and right sides
        else{
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        ...
        // increment both left and right sides by 1 on every increase of level
        else{
